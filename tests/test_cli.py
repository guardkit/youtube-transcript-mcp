"""Tests for CLI wrapper module (src/cli.py).

Covers argument parsing, JSON output, exit codes, stdin pipe support,
and command dispatch. Uses mocking for service calls to avoid network I/O.
"""
...
# ---------------------------------------------------------------------------
# AC-001: make_parser(), output_json(), exit_code_from_result(),
#         run_command(), main() exist as functions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AC-002: Subcommands registered
# ---------------------------------------------------------------------------


class TestSubcommands:
    """Test that all required subcommands are registered."""

    @pytest.fixture()
    def parser(self) -> MagicMock:
        return make_parser()
    ...
    def test_subcommand_registered(self, parser: MagicMock, command: str) -> None:
...
# ---------------------------------------------------------------------------
# AC-003: All stdout output is valid JSON via output_json() only
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AC-004: All logging goes to stderr (tested implicitly by not seeing
#         non-JSON on stdout in output tests above and below)
# ---------------------------------------------------------------------------


class TestLoggingToStderr:
    """Verify that logging does not pollute stdout."""
    ...
    def test_ping_stdout_is_pure_json(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        main(["ping"])
        captured = capsys.readouterr()
        # stdout must parse as JSON with no extra text
        data = json.loads(captured.out)
        assert isinstance(data, dict)


# ---------------------------------------------------------------------------
# AC-005: --help works on all subcommands (argparse raises SystemExit on -h)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AC-006: Exit code 0 on success, 1 on error
# ---------------------------------------------------------------------------


class TestExitCodes:
    """Test exit_code_from_result and main return correct exit codes."""
    ...
    def test_ping_returns_zero(self) -> None:
        code = main(["ping"])
        assert code == 0

    def test_list_focus_areas_returns_zero(self) -> None:
...
class TestStdinPipeSupport:
    ...
    def test_extract_insights_reads_stdin(
        ...
        fake_transcript = "A" * 200  # must be > 100 chars to pass validation
        ...
        data = json.loads(captured.out)
        assert code == 0
        # The prepare_for_extraction function should have received the stdin text
        assert data.get("transcript_length") == 200


# ---------------------------------------------------------------------------
# AC-008: get-transcript supports --language and --no-segments flags
# ---------------------------------------------------------------------------


class TestGetTranscriptFlags:
    """Test argument parsing for get-transcript flags."""
    ...
    def test_language_custom(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["get-transcript", "dQw4w9WgXcQ", "--language", "fr"])
        assert args.language == "fr"

    def test_no_segments_default_false(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["get-transcript", "dQw4w9WgXcQ"])
...
# ---------------------------------------------------------------------------
# AC-009: extract-insights supports --focus, --video-id, --max-insights
# ---------------------------------------------------------------------------


class TestExtractInsightsFlags:
    """Test argument parsing for extract-insights flags."""

    def test_focus_default(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["extract-insights", "text"])
        assert args.focus_areas == "general"

    def test_focus_custom(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["extract-insights", "text", "--focus", "youtube-channel"])
        assert args.focus_areas == "youtube-channel"

    def test_focus_multiple(self) -> None:
        parser = make_parser()
    ...
    def test_video_id_default(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["extract-insights", "text"])
        assert args.video_id == ""

    def test_video_id_custom(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["extract-insights", "text", "--video-id", "dQw4w9WgXcQ"])
        assert args.video_id == "dQw4w9WgXcQ"

    def test_max_insights_default(self) -> None:
        parser = make_parser()
        args = parser.parse_args(["extract-insights", "text"])
...
class TestPingCommand:
    """Test the ping command end-to-end."""
...
class TestListFocusAreasCommand:
    """Test the list-focus-areas command end-to-end."""
...
class TestExtractInsightsCommand:
    """Test the extract-insights command dispatch."""

    def test_extract_insights_with_text(self, capsys: pytest.CaptureFixture[str]) -> None:
        """Long enough transcript should produce extraction metadata."""
        text = "A" * 200
...
class TestVideoInfoErrorHandling:
    """Test error handling for video-info with invalid URL."""
...
# ---------------------------------------------------------------------------
# Mocked service dispatch tests for coverage of network-dependent branches
# ---------------------------------------------------------------------------


class TestGetTranscriptCommand:
    """Test get-transcript command with mocked TranscriptClient."""

    def test_get_transcript_invalid_url(self, capsys: pytest.CaptureFixture[str]) -> None:
        code = main(["get-transcript", "not-a-valid-url-at-all"])
        captured = capsys.readouterr()
        data = json.loads(captured.out)
        assert code == 1
        assert data["error"]["code"] == "INVALID_URL"
...
class TestListTranscriptsCommand:
    """Test list-transcripts command with mocked TranscriptClient."""

    def test_list_transcripts_invalid_url(self, capsys: pytest.CaptureFixture[str]) -> None:
        code = main(["list-transcripts", "not-valid"])
        captured = capsys.readouterr()
        data = json.loads(captured.out)
        assert code == 1
        assert data["error"]["code"] == "INVALID_URL"
...
class TestExtractInsightsWithVideoId:
    """Test extract-insights with --video-id flag in dispatch."""
